...
// ------------------------------------------------------------
// ⚠️ 这是**测试版**。它和公开版的 @name 不同，篡改猴当成两个独立脚本，能同时装上。
//    两个核心同时跑 = 双份部署/停采/喂食 + 两条通道互相抢 nonce，会白烧 gas。
//    装测试版之前，务必先在篡改猴面板把同类的**公开版停用**。
//
...
//       核心脚本      https://raw.githubusercontent.com/funcreator2030/kamigotchi-scripts/main/beta/kamigotchi-core-beta.user.js
//       辅助脚本      https://raw.githubusercontent.com/funcreator2030/kamigotchi-scripts/main/beta/kamigotchi-helper-beta.user.js
//       精简数据库    https://raw.githubusercontent.com/funcreator2030/kamigotchi-scripts/main/beta/kamigotchi-database-beta.user.js
//       轻量杀手监控  https://raw.githubusercontent.com/funcreator2030/kamigotchi-scripts/main/beta/kamigotchi-killer-monitor-beta.user.js
...
(async function () {
    ...
    // ============================================================
    // 【板块：单实例守卫】（0911 新增，测试线专属风险的兜底）
    // ------------------------------------------------------------
    // ▍为什么需要：从 0911 起有两条发行线——公开版（发给大家）和测试版
    //   （自己先跑几天）。两者 @name 不同，篡改猴当成两个独立脚本，
    //   **可以被同时启用**。一旦同时启用，同一个页面里就有两个核心在跑：
    //   双份部署、双份停采、双份喂食，两条 nonce 互相抢号——白烧 gas，
    //   严重时把对方的交易挤掉，等于没在保护 kami。
    // ▍做法：先加载的实例在 window 上插旗；后加载的看见旗子就整体不启动，
    //   并大声说明该去关掉哪一个。刷新页面 window 清空、旗子自然消失，
    //   所以不会误伤"刷新后重新注入"这种正常情况（iframe 也是独立 window，
    //   同样不会误报）。
    // ▍fail-open 铁律：只有**确实读到旗子**才拦。读旗子出任何异常，一律照常
    //   启动——宁可重复也不能让核心整个不跑；不跑就没人停采，kami 会被清算。
    // ▍这只是最后一道保险，不是省掉手动操作的理由：测试机上请把公开版核心
    //   在篡改猴面板里**停用**。
    // ============================================================
    const SCRIPT_VARIANT = '测试版';
    try {
        const __prevInst = window.__kamiCoreInstance;
        if (__prevInst && __prevInst.variant) {
            console.log(
                '%c⛔ 核心脚本【' + SCRIPT_VARIANT + '】本次不启动\n'
                + '   本页面已经有一个核心脚本【' + __prevInst.variant + ' v' + (__prevInst.version || '?') + '】在运行。\n'
                + '   两个核心同时跑 = 双份部署/停采/喂食 + nonce 互相抢号，会白烧 gas 甚至挤掉真正要发的交易。\n'
                + '   请在篡改猴面板里只保留一个核心脚本（测试期建议只开测试版），然后刷新页面。',
                'background:#c0392b;color:#fff;font-weight:bold;font-size:14px;padding:6px 10px;border-radius:4px;'
            );
            try { window.__kamiCoreDuplicateBlocked = { blocked: SCRIPT_VARIANT, running: __prevInst, at: Date.now() }; } catch (_) {}
            return;
        }
        window.__kamiCoreInstance = { variant: SCRIPT_VARIANT, version: null, at: Date.now() };
    } catch (_) { /* fail-open：守卫自身出问题，绝不阻断核心启动 */ }
    'use strict';
    ...
    window.showGasReport = async function () {
        ...
        try {
            ...
            const S = {
                ...
                win:     'color:#42a5f5;font-weight:bold',
                act:     'color:#7986cb',
                chainAct:'color:#ba68c8',
                life:    'color:#66bb6a;font-weight:bold;font-size:13px',
    ...
    // 🔻SYNC→内部版[1.2.16 定时gas报告]：每次页面刷新后 10 分钟自动跑一次 showGasReport()。
    //   核心每 ~45 分钟例行强制刷新 → 等效每 45 分钟自动出一份报告;Rollytics 拉取带 1h 缓存,
    //   多数轮次命中缓存零网络成本。只挂一次 setTimeout(非 interval),异常静默不影响主流程。
    setTimeout(() => {
        try {
            window.__kamiCallSource = 'auto_10min_timer';
            if (typeof window.showGasReport === 'function') window.showGasReport();
        } catch (_) {}
    }, 10 * 60 * 1000);
    ...
    // ▍可调参数：无。
    // ============================================================
    // 🔻SYNC[测试版1.2.47 版本标签防腐 + 构建时间自报]（用户 0913 提）
    // ------------------------------------------------------------
    ...
    (function versionCheck() {
        ...
        const SELF_VERSION = SCRIPT_VERSION;   // 🔻SYNC[测试版1.2.47] 引用顶部常量，不再单独维护
    ...
    setTimeout(() => {
        ...
        clog(`%c🧪 测试版核心运行中 v${SCRIPT_VERSION}（${SCRIPT_BUILT}）`+' —— 若同时看到「公开版」横幅，说明双开了，请去篡改猴停用其中一个',
            'background:#8e44ad;color:#fff;font-weight:bold;font-size:14px;padding:5px 10px;border-radius:4px;');
        clog(`%c🧪 Kamigotchi核心脚本-${SCRIPT_LINE} v${SCRIPT_VERSION}（${SCRIPT_BUILT}）可用命令（每条命令独占一行，直接复制粘贴）`, 'color: #8e44ad; font-weight: bold;');   // 🔻SYNC→内部版[1.1.17 可观测性批次]
        clog('══════════════════════════════════════════════════════════════');
        clog('');
        clog('───────── 🛑 紧急控制 ─────────');
    ...
    const EMERGENCY_CONFIG = {
        ...
        FEED_PHASE_CAP_MS: 90000,
        FEED_TAIL_GRACE_MS: 30000,        // api 补发段的额外宽限：主循环被截断后仍给补发留一点时间（喂食阶段总上限 = 90+30 = 120s）
    ...
    // 保护1: harvest.time.last超过24小时 → 疑似卡链。🔻SYNC[1.2.22] 改为"先试喂一次再判定"：
    //   首次遇到先照常喂食抢救（成本仅1笔tx+1个食物），并登记尝试时刻；6小时内若仍饿着才真跳过
    //   并提示用户手动处理（旧码无差别跳过，实测238只STARVING里232只从没被尝试过）
     ...
     */
    async function _starvingFeedKamis(kamiList, logPrefix, opts) {
    ...
    async function _starvingFeedKamisInner(kamiList, logPrefix, opts) {
        // 🔻SYNC[测试版1.2.43 ① 喂食堵停采] opts.deadlineMs = 本次喂食阶段的绝对截止时刻(Date.now()口径)。
        ...
        async function workerLoop() {
            while (true) {
                ...
                try {
                    ...
                    try {
                        ...
                        if (timeLast) {
                            ...
                            if (stuckDuration > STARVING_STUCK_TIME_MS) {
                                // 🔻SYNC→内部版[1.2.22 卡链先试一次]：用户 0825 定案——旧码对"链上>24h无tx"
                                //   直接跳过不喂,实测一次扫描 238 只 STARVING 里 232 只被无差别跳过、只喂了 6 只。
                                //   但"24h无tx"只是卡链的**嫌疑**不是铁证(也可能是长时间挂机后刚醒、索引滞后等),
                                //   一次喂食成本极低(1笔tx+1个食物),值得试;真喂不动再交给用户。
                                ...
                                // 🔻SYNC[1.2.24] 撤销 1.2.23 的单轮限流：用户指出 STARVING 是**活着的 0 血**
                                //   kami，杀手一来就没，和"已经死了可以慢慢救"的复活完全不同——**不能等下一轮**。
                                //   限流的初衷（防占锁数小时）改由"砍掉 estimateGas 预检 + 预分配 + 连续发送"解决。
                    ...
                    // 🔻SYNC→内部版[1.2.24 救援提速·砍estimateGas预检]（用户 0825 定案）
                    //   旧码这里对每只做一次 _preCheckStop（= estimateGas 真 RPC 调用），目的是
                    //   "能停采就别浪费食物"（DOM 0% 可能是取整误差）。实测代价：196 只 × RPC ÷ 并发8
                    //   ≈ **3 分钟**——期间这批 0 血 kami 一口食物没吃，杀手来了就是全灭。
                    //   取舍：STARVING 是"还活着但 0 血"，随时可被任何杀手秒杀，**及时性 >> 省几个食物**；
                    //   真是取整误差的，喂一口食物成本极低（金苹果库存 1500+）。故砍掉预检直接喂。
                    //   （其余 getByIndex 都是本地 ECS 1ms 级读取，不是瓶颈，保留。）
                } catch (e) {
        ...
        // 🔻SYNC→内部版[1.2.27 raw并行喂食+紧急级]（用户 0825 定案三件套:①批量分批连发喂食tx
        //   ②发完复查,到停采线的当场停 ③救援升级为紧急级）。
        //   为什么 raw:MUD 队列是串行的,每笔等前一笔确认或60s对账超时才放行(0825实测60~75s/笔,
        //   133只≈2小时);raw 直发+显式递增nonce=停采批量同款打法,133笔≈1~2分钟。
        ...
        const __feedChan = (() => { try { return localStorage.getItem('kami_starving_feed_channel') || 'queue'; } catch (_) { return 'queue'; } })();
        const __rawSigner = window.network?.network?.signer;
        ...
        let __emergencyLockToken = null;   // 🔻SYNC[测试版1.2.43 ② F3] 锁归属令牌，见下方释放处
        let __rawSent = 0;
        if (__rawMode) {
            try {
                // 🔻SYNC[测试版1.2.43 ② F2 nonce 读取窗口] 顺序从"读nonce → 加锁"改成"加锁 → 排空 → 读nonce"。
                //   BEFORE：先 getTransactionCount(pending) 拿到起点 N，再去设紧急锁。这中间是**裸窗口**——
                //           普通操作（部署/日常喂食，走 MUD 队列）此刻还没被紧急锁挡住，完全可以塞进一笔，
                //           把 N 占掉。等我们按 N 连发 raw，第一笔就撞号，后面整串跟着错位
                //           （0710 已定案过同型事故：停采批走原始签名器绕开 MUD 队列 nonce 通道 → 双通道分叉，46 次冲突）。
                //   AFTER ：先设紧急锁把普通通道关死，再等在飞的普通操作跑完（紧急停采那条路一直是这么做的），
                //           最后才读 nonce —— 读到的那一刻起就没人能再抢号了。
                if (!hasEmergencyLock()) {
                    setEmergencyLock();
                    __weSetEmergency = true;
                    __emergencyLockToken = window.__txEmergencyLock;   // 🔻SYNC[测试版1.2.43 ② F3]
                }
                if (window.__txNormalLock) {
                    log(`[TX锁] ⏳ [${logPrefix}] 等待普通操作 [${window.__txNormalLock.script}/${window.__txNormalLock.operation}] 让出通道后再读 nonce...`);
                    try { await waitForNormalLockRelease(30000); } catch (_) {}
                }
                ...
                // 🔻SYNC[测试版1.2.43 ② F2] 加锁已提到本 try 开头（读 nonce 之前），此处不再重复
        ...
        for (let idx = 0; idx < enriched.length; idx++) {
            ...
            // 🔻SYNC[1.2.24 全量预分配] 直接用 Step 2.5 预分配好的食物——零计算、零查询,
            //   连续发送不卡顿(分配规则见 Step 2.5:不浪费前提下加血最多)。
            const chosen = info.plannedFood || null;
            if (!chosen) {
                // 预分配阶段已判定无食物可用（Step 2.5 已打过"食物不足"红字提醒），这里静默跳过本只
                continue;
            }
            ...
            // 🔻SYNC[1.2.22 卡链先试一次]：明确告知用户"这是抢救性尝试"
            // 🔻SYNC[测试版1.2.43 ④(b) 6小时封锁错杀] 登记时刻已挪到**发送确实成功之后**（见下方 try 内）。
            //   BEFORE：这里先 set 再进 try 发送。发送本身抛错（raw nonce 错位 / 交易池满 /
            //           api 入口不可用 / RPC 超时）时没人撤销这个登记 —— 这只 kami 一口食物没吃到、
            //           一分 gas 没花，却被判"6 小时内已试过"，接下来六个小时的每一轮都直接
            //           skipReason='stuck24h' 跳过。断网夜整批发送失败 = 整批饿着锁死六小时。
            //   AFTER ：只有真的把 tx 交出去了才登记。发送失败 = 什么都没发生 = 下一轮照常再试
            //           （重试不烧 gas 也不烧食物，因为根本没发出去）。
            if (info.stuck24hTrying) {
                log(`%c🩺 [${logPrefix}] #${kami.dbIndex} 链上已 ${info.stuckHours} 小时无 tx（疑似卡链），先尝试喂食一次抢救；若本次无效，6 小时内不再重试，请手动处理`,
                    'color: #ffa726; font-weight: bold;');
            }
            ...
            try {
                ...
                // 🔻SYNC[测试版1.2.43 ② F1 同批不混通道] raw 批里遇到 Paeon 法术卡(11305)不再当场走 api。
                //   BEFORE：Paeon 只有 cast 入口、没有 raw 路径，于是在一串 raw 直发中间插一笔 MUD 队列 tx。
                //           两条通道各管各的 nonce（raw 用 getTransactionCount(pending) 自增，MUD 队列走自己的计数器），
                //           一交错就是 0710 那次 46 起冲突的同款分叉。
                //   AFTER ：把它登记进 __feedFailed，跟"发送失败待补发"的那些一起，在整个 raw 批发完之后
                //           由批尾那段**连续的 api 补发**统一发出去 —— 同一批里不再交错两条通道。
                //           它没被丢掉：补发段就在下面几行，同一次调用内一定会走到。
                if (__rawMode && isPaeonSpell) {
                    log(`↩️ [${logPrefix}] #${kami.dbIndex} 选中 ${chosen.name}(法术卡,只有 api 入口)，挪到批尾 api 段统一发，避免与 raw 混用 nonce`);
                ...
                if (__willUseRaw) {
                    ...
                    // Paeon法术卡走cast、或用户切回queue通道:保留MUD队列路径
                    log(`🍔 [${logPrefix}] 喂食 #${kami.dbIndex} → ${chosen.name}(+${chosen.hp}HP) [${apiName}] (fire-and-forget)`);
                    __feedTx = await apiFn(info.kamiId, chosen.index);
                }
                _gasLedgerRecord('feed', [info.kamiId], __feedTx);   // 🔻SYNC[1.2.7 gas真值账本]

                fedCount++;
                // 🔻SYNC[测试版1.2.43 ④(a) 喂后复查] 记下本笔 hash，供下方复查判定"tx 到底落没落地"
                try { info.__feedTxHash = __feedTx && __feedTx.hash ? String(__feedTx.hash) : null; } catch (_) { info.__feedTxHash = null; }

                // 🔻SYNC[测试版1.2.43 ④(b) 6小时封锁错杀] 登记"本轮已抢救过"——只在这里登记，
                //   即发送确实成功之后。另外：queue 通道的 await 要等上链才 resolve，拿到的是
                //   receipt 形（v1.1.30 实测），能直接看出 status；status 明确为 0 = 链上 revert =
                //   这只 kami 并没吃到东西，那就不该登记，下一轮还得再救。
                ...
                // 🔻SYNC[1.2.27] 发送节奏:raw=150ms/笔+每10笔歇1.2s(批量分批);queue=300ms/笔
        ...
        if (__feedFailed.length > 0) {
            log(`%c🔁 [${logPrefix}] ${__feedFailed.length} 只待 api 补发（发送失败的 + raw 批里挪过来的法术卡），逐只补发一次（api 是跑了几个月的稳定路径）`,
                'color: #ffa726; font-weight: bold;');
            ...
            for (const f of __feedFailed) {
                ...
                try {
                    ...
                    fedCount++;
                    __retryOk++;
                    // 🔻SYNC[测试版1.2.43 ④(a)] 补发的也要留 hash，否则喂后复查一律判"未落地"，
                    //   转停覆盖率会随 ②F1 把法术卡挪进来而变差
        ...
        //   (喂食tx落地会把 stats.health.sync 刷新为真实HP,KB§18.12——此刻sync可信);
        //   喂活且仍在停采线下的→翻isStarving+更新delta,紧随其后的停采批自然带走(复用全部现有停采机器);
        //   喂活且血量健康的→delta>1,被下游过滤,继续采集;tx未落地的→保持只喂不停,下轮再救。
        // 🔻SYNC[测试版1.2.43 ④(a) 喂后复查误判] 上面那句"tx未落地的→保持只喂不停"原来并没有实现:
        //   代码只看 sync>0，而 sync 是**上一次 tx 时的检查点**（KB§18.12）。0 血 kami 的旧检查点
        //   完全可能是满血——部署时 100，之后一路流干到 0，链上 sync 仍写着 100。
        //   于是 tx 还堵在池子里没上链时，这里读到 sync=100 → 判"喂活" → isStarving 翻 false →
        //   delta 也跟着算出来 → 紧随其后的停采批把这只 0 血 kami 一起带走 → 链上必 revert
        //   （0825 实测 10/10 CALL_EXCEPTION，1.2.26 建停采名单剔除就是为了防这个）。
        //   修法：先按 hash 查 receipt 拿链上实锤，status===1 才允许信 sync；
        //   查不到 receipt / status=0 / 没有 hash → 一律保持 isStarving，交下一轮，
        //   即"宁可多饿一轮，也不把 0 血 kami 塞进停采批"。
        if (__rawMode && fedCount > 0) {
            try {
                const __postWaitMs = 20000;
                log(`⏳ [${logPrefix}] 已连发 ${fedCount} 笔,等待 ${__postWaitMs / 1000}s 确认后复查血量...`);
                await delay(__postWaitMs);
                const __rcProvider = _gasLedgerProvider();   // 🔻SYNC[测试版1.2.43 ④(a)] 只读 provider，绝不发 tx
                let __alive = 0, __toStop = 0, __still0 = 0, __notLanded = 0;
                for (const info of enriched) {
                    if (!info || !info.plannedFood || info.skipReason) continue;
                    try {
                        // 🔻SYNC[测试版1.2.43 ④(a)]【落地实锤】没 hash / 查不到 receipt / status≠1 → 这笔没成，sync 不可信
                        let __landed = false;
                        if (info.__feedTxHash && __rcProvider && typeof __rcProvider.getTransactionReceipt === 'function') {
                            try {
                                const __rc = await __rcProvider.getTransactionReceipt(info.__feedTxHash);
                                __landed = !!(__rc && Number(__rc.status) === 1);
                        ...
                        if (sync > 0 && total > 0) {
                            __alive++;
                            const pct = sync / total * 100;
                            const thr = Number(info.kami.threshold);
                            if (Number.isFinite(thr)) {
                                info.kami.hpPercent = pct;
                                info.kami.delta = pct - thr;
                                info.kami.isStarving = false;   // 喂活:回归正常逻辑
                                if (pct - thr <= 1) __toStop++;
                            }
                        } else { __still0++; }
                    } catch (_) {}
                }
                if (__notLanded > 0) {   // 🔻SYNC[测试版1.2.43 ④(a)]
                    log(`⏳ [${logPrefix}] 喂后复查:${__notLanded} 只的喂食tx ${__postWaitMs / 1000}s 内未见 receipt 实锤，保持只喂不停（不进停采批），下轮再救`);
                }
                log(`%c🩺 [${logPrefix}] 喂后复查:${__alive} 只已确认喂活(${__toStop} 只在停采线下,转入本轮停采),${__still0} 只确认未到账(继续只喂不停,下轮自动再救)`,
                    'color: #66bb6a; font-weight: bold;');
            } catch (e) { log(`⚠️ [${logPrefix}] 喂后复查异常(${e?.message || e}),不影响已发喂食;未复查的下轮扫描自然处理`); }
        }
        // 🔻SYNC[测试版1.2.43 ② F3 锁归属] 只释放**我们自己设的那把**锁。
        //   BEFORE：只看 __weSetEmergency 这个布尔。如果我们持锁期间锁被别人释放又重设
        //           （比如 720s 强制释放兜底触发后紧急停采重新取锁），这里会把别人正在用的锁掐掉。
        //   AFTER ：比对取锁时留下的对象引用，不是同一把就不动它（并留一条日志，便于事后查）。
        if (__weSetEmergency) {
            try {
                if (!__emergencyLockToken || window.__txEmergencyLock === __emergencyLockToken) {
                    releaseEmergencyLock();
                } else {
                    log(`[TX锁] ⚠️ [${logPrefix}] 紧急锁已被他人重设，本轮不释放（避免掐掉别人的锁）`);
                }
            } catch (_) {}
        }
    ...
    window['安装说明'] = window.showKamiInstall = function () {
        ...
        clog('   当前这份：测试版 核心 v' + (window.__kamiCoreVersion || '?'));
        clog('══════════════════════════════════════════════════════════════');
        clog('');
        clog('%c⚠️ 这是测试版。公开版必须在篡改猴里停用，否则两个核心同时跑=双份交易+抢 nonce', 'color: #e67e22; font-weight: bold;');
        clog('');
        ...
        clog('      核心脚本      https://raw.githubusercontent.com/funcreator2030/kamigotchi-scripts/main/beta/kamigotchi-core-beta.user.js');
        clog('      辅助脚本      https://raw.githubusercontent.com/funcreator2030/kamigotchi-scripts/main/beta/kamigotchi-helper-beta.user.js');
        clog('      精简数据库    https://raw.githubusercontent.com/funcreator2030/kamigotchi-scripts/main/beta/kamigotchi-database-beta.user.js');
        clog('      轻量杀手监控  https://raw.githubusercontent.com/funcreator2030/kamigotchi-scripts/main/beta/kamigotchi-killer-monitor-beta.user.js');
    ...
    async function emergencyStopHarvest(opts) {
        ...
        try {
            ...
            if (starvingList.length > 0) {
                ...
                // 🔻SYNC[测试版1.2.43 ① 喂食堵停采] 传 deadlineMs 给喂食阶段封顶：
                //   0 血 kami 只有喂食能救（停采链上必 revert，1.2.26 已把它们剔出停采名单），
                //   所以喂食仍然排在停采前面、一秒都不推迟——用户 0825 定的
                //   「饿死的不能等下一轮」原样保留。这里封的只是**尾巴**：
                //   喂不完的交下一轮，换来后面停采循环一定拿得到自己的预算。
                const fedCount = await _starvingFeedKamis(starvingList, '紧急/饿死救援',
                    { deadlineMs: Date.now() + EMERGENCY_CONFIG.FEED_PHASE_CAP_MS });
            ...
            // 🔻SYNC[测试版1.2.43 ① 喂食堵停采] 停采阶段独立时钟。
            //   BEFORE：下面两处硬上限都拿 startTime 比，喂食花掉的时间直接从停采预算里扣，
            //           喂食超过 180s 时停采 while 循环第一次判定就 break —— 一批 tx 都没发出去，
            //           日志上只留一句"达到硬上限"，看起来像正常收尾，实际是整轮停采被吞了。
            //   AFTER ：喂食阶段自己有 FEED_PHASE_CAP_MS 封顶（≤120s 含补发宽限），
            //           停采阶段从这里重新起算，无论喂了多久都拿得到完整 180s。
            //   锁占用上界：120(喂) + 180(停) + 93(cooldown补停) ≈ 393s，仍远低于紧急锁 720s 强制释放线。
            //   注意：日志汇总里的"总耗时"仍用 startTime，保持真实口径不变。
            const __stopPhaseStart = Date.now();

            // 🔻SYNC→内部版[1.2.26 STARVING只喂不停]（用户 0825 定案+链上实证）：0血kami停采
            //   链上必revert(0825实测10/10 CALL_EXCEPTION)——停采名单必须剔除STARVING。
            //   它们只走喂食救援:喂活(+150HP,约79%)后高于停采线,继续采集即健康状态,
            //   下轮扫描自然回归正常逻辑。此过滤同时化解1.2.23互斥的副作用
            //   (互斥跳过喂食时,调用方曾把"跳过"当"喂完"直接停采0血kami→16批必败tx)。
            const __stopListNoStarv = validList.filter(x => !x.isStarving && !(typeof x.delta === 'number' && x.delta > 1));   // 🔻SYNC[1.2.27] 喂活转健康的也不停
            const __starvSkipN = validList.length - __stopListNoStarv.length;
            if (__starvSkipN > 0) {
                log(`%c⏸️ [紧急停采] ${__starvSkipN} 只 STARVING 已交喂食救援，本轮不停采（0血停不了采；喂活后下轮按正常逻辑处理）`,
                    'color: #ffa726; font-weight: bold;');
            }
            // Step 4.6: 喂食后重新预检链上状态，确认哪些kami仍需停采（用户可能已手动停采）
            log(`🔍 [紧急停采] 重新预检链上状态（喂食后/停采前）...`);
            const recheck = await _emergencyPreCheck(__stopListNoStarv);
            ...
            while (remaining.length > 0 && round <= EMERGENCY_CONFIG.MAX_ROUNDS) {
                ...
                const elapsedSoFar = Date.now() - __stopPhaseStart;   // 🔻SYNC[测试版1.2.43 ①] 从喂食结束起算
            ...
            if (cooldownDeferred.size > 0 && (Date.now() - __stopPhaseStart) > EMERGENCY_CONFIG.INVOCATION_HARD_CAP_MS) {   // 🔻SYNC[测试版1.2.43 ①] 同上，从喂食结束起算
                log(`%c⏰ [停采诊断] 停采阶段已耗时 ${Date.now() - __stopPhaseStart}ms（本次调用总 ${Date.now() - startTime}ms），达到硬上限，跳过 cooldown 收尾（${cooldownDeferred.size} 个交下一轮）`,
                    'color: red; font-weight: bold;');
    ...
    //   默认仍要求按钮；紧急停采只走 API 发交易、从不点按钮，用 requireButton:false —— 否则列表一折叠
    //   它就整晚"无法判断"，一只都不停（0913 23:23 / 23:28 实录）。
    function __countIncompleteDom(list, opts) {
        const requireButton = !(opts && opts.requireButton === false);
        let incomplete = 0;
        for (const kd of list) {
            ...
            const mB = requireButton ? kd.querySelector('img[src*="/assets/harvest-"], img[src*="/assets/stop-"]')?.closest('button') : true;
    ...
    async function __emergencyScanWithDomGuard(tag) {
        // 判据与主循环同源（__countIncompleteDom），但不要求主按钮（见其注释：折叠列表时按钮不渲染而血量仍可读）。
        ...
        for (let t = 0; t < MAX_TRIES; t++) {
            ...
            const incomplete = n > 0 ? __countIncompleteDom(cards, { requireButton: false }) : 0;   // 🔻SYNC[测试版1.2.52] 紧急停采不点按钮，折叠列表也能读血量
    ...
    //     DB 缺记录、或 DB 里的 harvestId 缺失/非法时，作为实时数据
    //     的回退来源
    // ▍触发时机：
    ...
    //      harvestId；查询失败的保守保留。
    //      🔻SYNC[0913 实测勘误] 原注释写「harvestId 重新部署后会变化」——**错的**。
    //      0913 用建库时存的 harvestId 与当前链上值对比：**两账户合计 354 只在采 kami
    //      （142 + 212），跨 2 个地块（74 / 18），100% 一致、0 例外**——而这批 kami
    //      建库后已停采/重部署上百轮。与 0709 实盘「harvestId 对每只固定不变、
    //      可缓存、可当前端冻结 canary」一致。这里每次重刷是**兜底**（防 DB 记录缺失/损坏），
    //      不是因为它会变。⚠️ 别再据此写「不能缓存 harvestId」之类的结论。
    ...
    async function runAutomation() {
        ...
        //   影响面更大；退避重试上限 5 次
        // ============================================================
        // 🔻SYNC[1.2.26 STARVING只喂不停]：同紧急路径——0血停采必revert,API批只停非STARVING
        const __apiNoStarv = apiList.filter(x => !x.isStarving && !(typeof x.delta === 'number' && x.delta > 1));   // 🔻SYNC[1.2.27] 喂活转健康的(delta>1)也不停
        if (apiList.length - __apiNoStarv.length > 0) {
            log(`%c⏸️ [批量停止] ${apiList.length - __apiNoStarv.length} 只 STARVING 已交喂食救援，本轮不停采（0血停不了采）`,
                'color: #ffa726; font-weight: bold;');
        }
        const apiBatches = chunkRandom(__apiNoStarv, 6, 10);
        let needDomAll = [];
        ...
        // ============================================================
        // 🔻SYNC[1.2.26 STARVING只喂不停] DOM 停采同样剔除 STARVING(0血点停采按钮同样无效)
        const domTargets = [...domList, ...needDomAll].filter(it => it.delta <= 1 && !it.isStarving);
        const seen = new Set();
    ...
    const REVIVE_MAX_PER_ROUND = 30;   // 🔻SYNC[1.2.21] 单轮复活上限：防批量复活长时间独占TX锁阻塞喂食/停采/部署（0815雪崩教训）；剩余下轮自动继续
    ...
    async function _reviveDeadBatch(deadList) {
        ...
        const byRibbon = fresh.slice(0, ribbons);   // 丝带不足时截断：只复活名单前 N 只（N = 丝带余额）
        if (byRibbon.length < fresh.length) {
            log(`%c⚠️ [复活] 丝带仅 ${ribbons} 个 < 死亡 ${fresh.length} 只，本轮先复活前 ${byRibbon.length} 只，请补货`, 'color: orange; font-weight: bold;');
        }
        // 🔻SYNC→内部版[1.2.21 复活单轮限流]：单轮复活上限，防长时间独占 TX 锁。
        //   动因(0815 实盘 1105 只死亡雪崩)：一次批量复活 215 只 = 215 笔 tx ≈ 10~18 分钟，
        //   期间普通锁被独占，喂食/部署/普通停采/拾荒全部排队(实测普通锁被超时强制释放)，
        //   反而制造新的死亡。分轮跑：每轮锁占用降到约 1.5 分钟，其余模块能正常穿插。
        //   剩余的由下一轮死亡监控(约3分钟)自动继续，15 分钟防重发冷却保证不会重复发 tx。
        const todo = byRibbon.slice(0, REVIVE_MAX_PER_ROUND);
        if (todo.length < byRibbon.length) {
            const __left = byRibbon.length - todo.length;
            const __rounds = Math.ceil(byRibbon.length / REVIVE_MAX_PER_ROUND);
            log(`%c🧯 [复活/单轮限流] 待复活 ${byRibbon.length} 只 → 本轮先救 ${todo.length} 只（单轮上限 ${REVIVE_MAX_PER_ROUND}）`,
                'color: #42a5f5; font-weight: bold;');
            log(`   为什么限流：一次性复活几百只会独占 TX 锁十几分钟，期间喂食/停采/部署全被卡住，反而害死更多 kami（0815 实盘教训）`);
            log(`%c   剩余 ${__left} 只：下一轮死亡扫描（约 3 分钟后）自动继续，预计 ${__rounds} 轮全部救回；丝带不会浪费，也不会重复发 tx，无需人工干预`,
                'color: #66bb6a;');
        }
    ...
    /**
     * 🔻SYNC[测试版1.2.37] 两处改动：
     *   ① **返回布尔**——调用方要靠它决定"这 50 步长到底扣不扣"（A09 修复需要）。
     *      官方队列的 sendTx 等成功回执才 resolve、失败抛错，所以没抛即成功。
     *      原来什么都不返回，调用方拿到 undefined → 永远当失败 → 步长本地不扣 → 低估 → 白白跳过后续合成。
     *   ② 补上 gas 真值账本 hook——ChatGPT 审计 A14 指出核心的两处合成从来没记账，
     *      导致"账本攒够一天就当完整"的判据本身建立在残缺样本上。
     * @returns {Promise<boolean>} 是否合成成功
     */
    async function autoCraftGreaterXPPotion() {
        log("🔧 开始合成 Greater XP Potion");
        try {
            const __tx = await window.network.api.player.account.item.craft(2, 1);   // 配方编号 2 = Greater XP Potion，执行 1 次
            _gasLedgerRecord('craft', [2], __tx);   // 🔻SYNC[1.2.37 A14] 补记账
    ...
    async function autoXPPotionFlow() {
        ...
        if (stamina >= 50 &&
            ...
            log("🔧 开始合成 Greater XP Potion");
            const _craftOk = await autoCraftGreaterXPPotion();
            // 🔻SYNC[测试版1.2.37 A09] 合成**确认成功**才在本地扣掉这 50 步长。
            //   官方队列的 sendTx 等到成功回执才 resolve、失败会抛，所以没抛即成功。
            if (_craftOk) stamina = Math.max(0, stamina - 50);
        ...
        if (currentStamina >= POLLEN_BATCH_STAM &&
            ...
            try {
                ...
                const __ptx = await window.network.api.player.account.item.craft(6, POLLEN_BATCH);
                _gasLedgerRecord('craft', [6], __ptx);   // 🔻SYNC[1.2.37 A14] 补记账（原来这笔从不入账本）
